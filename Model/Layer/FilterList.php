...
namespace AumTechnolabs\AmastyURLAliasToSiteMap\Model\Layer;
...
class FilterList extends  \Magento\Catalog\Model\Layer\FilterList
{
    ...
    public function __construct(
        ...
        \Amasty\Shopby\Model\Layer\Category\FilterableAttributeList $filterableAttributes,
        ...
        if (version_compare($version, self::VERSION24, '>=')) {
            $params = [
                ...
                [
                    "attribute" => \AumTechnolabs\AmastyURLAliasToSiteMap\Model\Layer\Filter\Attribute::class,
                    "price" => \AumTechnolabs\AmastyURLAliasToSiteMap\Model\Layer\Filter\Price::class,
                    "decimal" => \AumTechnolabs\AmastyURLAliasToSiteMap\Model\Layer\Filter\Decimal::class,
                    "category" => \AumTechnolabs\AmastyURLAliasToSiteMap\Model\Layer\Filter\Category::class
                ]
            ];
        } else {
            $params = [
    ...
    protected function checkFilterVisibility(FilterSettingInterface $setting, $currentCategoryId)
    {
        $visible = true;
        if (
            $setting->getVisibleInCategories() === VisibleInCategory::ONLY_IN_SELECTED_CATEGORIES
        ...
        if (
            $setting->getVisibleInCategories() === VisibleInCategory::HIDE_IN_SELECTED_CATEGORIES
            && in_array($currentCategoryId, $setting->getCategoriesFilter())
        ) {
            $visible = false;
        }
